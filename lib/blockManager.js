...
function calculatePPLNSPayments(blockHeader) {
    ...
    let coefForETN = 1000 * 1000 * 100; // 100 million
    let rewardTotal = blockHeader.reward * coefForETN;
    ...
    console.log(`\n`);
    console.log(`_______________________________________ Вызван calculatePPLNSPayments`);
    console.log(`paymentData`);
    console.log(paymentData);
    console.log(`--------------------`);
    console.log(`paymentData[global.config.payout.feeAddress]`);
    console.log(paymentData[global.config.payout.feeAddress]);
    console.log(`\n`);
    ...
    async.doWhilst(function (callback) {
            ...
            cursor.getCurrentBinary(function (key, data) {  // jshint ignore:line
                ...
                if (shareData.poolType === global.protos.POOLTYPE.PPLNS) {
                    ...
                    if (totalPaid + amountToPay > rewardTotal) {
                        ...
                        console.log(`totalPaid + amountToPay > rewardTotal`);
                        console.log(totalPaid + amountToPay > rewardTotal);
                        console.log(amountToPay);
                    }
                    ...
                    console.log(`paymentData[global.config.payout.feeAddress].amount`);
                    console.log(paymentData[global.config.payout.feeAddress].amount);
                    console.log(`feesToPay`);
                    console.log(feesToPay);
        ...
        console.log("Decrementing the block chain check height to:" + blockCheckHeight);
        if (totalPayment >= rewardTotal) {
            console.log("Loop 1: Total Payment: " + totalPayment + " Amount Paid: " + rewardTotal + " Amount Total: " + totalPaid);
            return false;
        } else {
            console.log("Loop 2: Total Payment: " + totalPayment + " Amount Paid: " + rewardTotal + " Amount Total: " + totalPaid);
            return blockCheckHeight !== 0;
        }
    }, function (err) {
        let totalPayments = 0;
        Object.keys(paymentData).forEach(function (key) {
            paymentData[key].amount = Math.floor(paymentData[key].amount / coefForETN);
            balanceQueue.push(paymentData[key], function () {
            });
            totalPayments += paymentData[key].amount;
